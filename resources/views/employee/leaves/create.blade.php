...
<div class="mx-auto px-4 py-8 max-w-5xl">
    ...
    @if (session('success'))
        <div class="bg-green-50 border border-green-200 rounded-lg shadow-md p-6 mb-8">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-green-900">{{ session('success') }}</h3>
                </div>
            </div>
        </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Leave Application</h1>
                <p class="text-gray-600 mt-2">CS Form No. 6, Revised 2020</p>
            </div>
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                <p class="text-sm font-medium text-blue-900">
                    <i class="fas fa-calendar text-blue-600 mr-2"></i>
                    Filing Date: <span class="font-bold">{{ now()->format('M d, Y') }}</span>
                </p>
            </div>
        </div>
        <p class="text-gray-700">
            Complete all required fields below. Fields marked with <span class="text-red-600 font-bold">*</span> are mandatory.
        </p>
    </div>

    <form action="{{ route('employees.leaves.store') }}" method="POST" x-data="{ ...leaveApplicationForm(), ...signatureUploader() }" enctype="multipart/form-data">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="middle_name" id="middle_name" readonly
                        class="w-full px-4 py-2 border border-gray-300 rounded-md bg-gray-100 text-gray-700 cursor-not-allowed"
                        value="{{ old('middle_name', auth()->user()->personalInformation?->middle_name) }}">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">
                    <i class="fas fa-info-circle mr-1"></i> Your position is auto-loaded from your HR profile
                </p>
            </div>
        </div>
            ...
            <div class="max-w-md">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="salary" id="salary" required readonly step="0.01" min="0"
                        ...
                        value="{{ old('salary', auth()->user()->personalInformation?->salary ?? 0) }}">
                </div>
                <p class="text-xs text-gray-500 mt-1">
                    <i class="fas fa-info-circle mr-1"></i> Salary retrieved from your HR profile
                </p>
            </div>
        </div>
                    ...
                    <template x-if="leaveType && !['vacation', 'sick', 'special_privilege', 'study', ''].includes(leaveType)">
                        <div>
                            ...
                            <textarea name="other_leave_details" id="other_leave_details" rows="3"
                                class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                placeholder="Provide details relevant to your leave type...">{{ old('other_leave_details') }}</textarea>
                        </div>
                    </template>
                </div>
            </div>
...
function leaveApplicationForm() {
    return {
        ...
        halfDayTime: '{{ old('half_day_time', 'morning') }}',
        locationWithinPhilippines: '{{ old('location_type', 'within') }}',
        sickType: '{{ old('sick_type', '') }}',
        studyPurpose: '{{ old('study_purpose', '') }}',
        slpType: '{{ old('slp_type', '') }}',
        ...
        onLeaveTypeChange() {
            ...
            this.studyPurpose = '';
            this.slpType = '';
            this.locationWithinPhilippines = 'within';
        },
