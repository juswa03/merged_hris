...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Travel Request Funds Review</h1>
            <a href="{{ route('accountant.travel.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg transition">
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            @if($financeApproval)
                <div class="mb-4">
                    <h3 class="font-semibold text-gray-800 mb-2">1. Finance Officer Review</h3>
                            ...
                            <div>
                                ...
                                @if($financeApproval->status === 'approved')
                                    <div class="text-sm text-gray-700 space-y-1">
                                        <p><strong>Approved by:</strong> {{ $financeApproval->approver->first_name ?? 'N/A' }} {{ $financeApproval->approver->last_name ?? '' }}</p>
                                ...
                                @elseif($financeApproval->status === 'rejected')
                                    <div class="bg-red-100 border border-red-300 rounded-lg p-3 mt-2">
                                        <p class="text-red-800 text-sm"><strong>Rejection Reason:</strong></p>
                                        <p class="text-red-700 text-sm">{{ $financeApproval->rejection_reason }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
            ...
            <!-- Stage 2: Accountant (Current) -->
            <div>
                <h3 class="font-semibold text-gray-800 mb-2">2. Accountant Review (Your Decision)</h3>
                <div class="bg-purple-50 border border-purple-200 rounded-lg p-4">
                    <div class="flex items-start">
                        <div class="flex-shrink-0">
                            @if($accountantApproval->status === 'pending')
                                <i class="fas fa-hourglass-start text-orange-500 text-2xl mr-4"></i>
                            @elseif($accountantApproval->status === 'approved')
                                <i class="fas fa-check-circle text-green-500 text-2xl mr-4"></i>
                            @else
                                <i class="fas fa-times-circle text-red-500 text-2xl mr-4"></i>
                            @endif
                        ...
                        <div>
                            <h4 class="font-semibold text-gray-900 mb-2">
                                Status: <span class="capitalize">{{ $accountantApproval->status }}</span>
                            </h4>
                            
                            @if($accountantApproval->status === 'pending')
                                <p class="text-sm text-gray-700">Awaiting your review and approval</p>
                            @elseif($accountantApproval->status === 'approved')
                                <div class="text-sm text-gray-700 space-y-1">
                                    <p><strong>Approved by:</strong> {{ $accountantApproval->approver->first_name ?? 'N/A' }} {{ $accountantApproval->approver->last_name ?? '' }}</p>
                                    @if($accountantApproval->approved_at)
                                        <p><strong>Approved at:</strong> {{ $accountantApproval->approved_at->format('M d, Y g:i A') }}</p>
                                    @endif
                                    @if($accountantApproval->comments)
                                        <p><strong>Comments:</strong> {{ $accountantApproval->comments }}</p>
                                    @endif
                                </div>
                            @elseif($accountantApproval->status === 'rejected')
                                <div class="bg-red-100 border border-red-300 rounded-lg p-3 mt-2">
                                    <p class="text-red-800 text-sm"><strong>Rejection Reason:</strong></p>
                                    <p class="text-red-700 text-sm">{{ $accountantApproval->rejection_reason }}</p>
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        @if($canApprove)
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('accountant.travel.approve', $travel) }}" method="POST" enctype="multipart/form-data" x-data="accountantApprovalForm()" @submit.prevent="submitForm">
                    @csrf
                    ...
                    <!-- Funds Available (conditional - only for approval) -->
                    <div x-show="status === 'approved'" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                        <label for="funds_available" class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-check-circle text-green-600 mr-2"></i>
                            Funds Available <span class="text-red-600">*</span>
                        </label>
                        <input type="number" name="funds_available" id="funds_available" 
                               step="0.01" min="0" max="99999999.99"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                               placeholder="Enter the available travel funds amount..."
                               x-bind:required="status === 'approved'"
                               value="{{ old('funds_available', '') }}">
                        <p class="text-xs text-gray-600 mt-1">The amount of funds available in the budget for this request</p>
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                        <div class="space-y-2">
                            <p class="text-sm text-blue-800">
                                ...
                                Your decision will be recorded and the employee will be notified. If approved, the request will move to the Department Head approval stage.
                            </p>
                            <p class="text-sm text-blue-800">
                                <i class="fas fa-asterisk text-red-600 mr-2"></i>
                                <strong>Required Fields:</strong> Decision, Funds Available Amount (if approved), and E-Signature
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-4">
                        <a href="{{ route('accountant.travel.index') }}" 
                        ...
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
                                :disabled="!status || !signatureData || (status === 'approved' && !document.getElementById('funds_available').value)">
                            <span x-show="status !== 'rejected'">
                                <i class="fas fa-check mr-2"></i> Approve & Send to Department Head
                            </span>
                            <span x-show="status === 'rejected'">
            ...
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 text-sm mt-1">This travel request has already been reviewed at the accountant stage.</p>
                    </div>
                </div>
            </div>
        @endif
...
@push('scripts')
<script>
function accountantApprovalForm() {
    return {
        status: '{{ old("status", "") }}',
        isDragging: false,
        signatureData: '',
        ...
        submitForm() {
            ...
            if (this.status === 'approved') {
                const fundsField = document.getElementById('funds_available');
                if (!fundsField.value) {
                    alert('Please enter the funds available amount');
                    return;
                }
            }
