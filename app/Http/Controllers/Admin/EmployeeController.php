...
{
    $employees = Employee::with(['department', 'user', 'position', 'jobStatus'])
        ->orderBy('created_at', 'desc')
        ->get();

    $departments = Department::orderBy('name')->pluck('name');

    // Use DB-level counts instead of in-memory collection filtering for performance
    $stats = [
        'total'      => Employee::count(),
        'active'     => Employee::whereHas('jobStatus', fn($q) => $q->where('name', 'Active'))->count(),
        'on_leave'   => Employee::whereHas('jobStatus', fn($q) => $q->where('name', 'On Leave'))->count(),
        'terminated' => Employee::whereHas('jobStatus', fn($q) => $q->whereIn('name', ['Terminated', 'Suspended', 'Inactive']))->count(),
    ];
