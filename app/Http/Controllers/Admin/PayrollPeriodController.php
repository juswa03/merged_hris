...
class PayrollPeriodController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.payroll.periods.index')->with('success', 'Payroll period created successfully.');
    }
    ...
    public function update(Request $request, PayrollPeriod $period)
    {
        ...
        return redirect()->route('admin.payroll.periods.index')->with('success', 'Payroll period status updated successfully.');
    }

    public function destroy(PayrollPeriod $period)
    {
        if ($period->payrolls()->count() > 0) {
            return redirect()->route('admin.payroll.periods.index')->with('error', 'Cannot delete period with existing payroll records.');
        }

        $period->delete();
        return redirect()->route('admin.payroll.periods.index')->with('success', 'Payroll period deleted successfully.');
    }
}
