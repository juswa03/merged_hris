...
class DtrController extends Controller
{
   ...
   public function index(Request $request)
    ...
    public function export(Request $request)
    {
        ...
        // Generate CSV
        $filename = 'DTR_' . $selectedDate->format('Y_m') . '_' . now()->format('YmdHis') . '.csv';
        ...
        $callback = function() use ($dtrEntries) {
            ...
            foreach ($dtrEntries as $entry) {
                fputcsv($file, [
                    ...
                    $entry->employee->position->title ?? 'N/A',
                    Carbon::parse($entry->dtr_date)->format('Y-m-d'),
                    Carbon::parse($entry->dtr_date)->format('l'),
                    $entry->am_arrival ?? '',
                    $entry->am_departure ?? '',
                    $entry->pm_arrival ?? '',
                    $entry->pm_departure ?? '',
    ...
    public function exportCsForm48(Request $request, $employeeId)
    {
        ...
        foreach ($period as $date) {
            ...
            $daysInMonth[] = [
                ...
                'undertime_hours' => $dtr ? $this->formatUndertimePreview($dtr->under_time_minutes) : '',
                'remarks' => $dtr->remarks ?? '',
            ];
        }
